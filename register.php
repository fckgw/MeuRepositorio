...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($password !== $password_confirm) { $error = "As palavras-passe não coincidem."; }
    elseif (strlen($password) < 6) { $error = "A palavra-passe deve ter pelo menos 6 caracteres."; }
    else {
        ...
        if ($stmt->execute()) {
            ...
            exit();
        } else { $error = 'Email ou nome de utilizador já existe.'; }
        $stmt->close();
    }
}
$conn->close();
